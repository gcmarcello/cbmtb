...
router.post(
  "/register",
  [reCaptcha, registrationValidation],
  async (req, res) => {
    const {
      address,
      apartment,
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Cadastro efetuado com sucesso.", type: "success" });
    } catch (err) {
      console.log(err.message);
      res.status(500).json({
        message:
          "Erro realizar o cadastro. por favor atualize a página e tente novamente.",
        type: "error",
      });
    }
  }
...
router.put("/", [authorization, registrationValidation], async (req, res) => {
  ...
  try {
    if (!(await bcrypt.compare(password, userInfo.rows[0].user_password))) {
      return res
        .status(403)
        .json({ message: "Senha incorreta.", type: "error" });
    }
...
router.put("/admin", adminAuthorization(), async (req, res) => {
  ...
  try {
    const userInfoEmail = (
      await pool.query("SELECT user_email FROM users WHERE user_id = $1", [
        userId,
      ])
    ).rows[0];
    ...
    if (userInfoEmail.user_email !== email) {
      const deactivateAccount = await pool.query(
        "UPDATE users SET user_confirmed = $1 WHERE user_id = $2",
        [false, userId]
      );
      const newConfirmation = await pool.query(
        "INSERT INTO email_confirmations (register_date,user_id,confirmation_status) VALUES ($1,$2,$3) RETURNING *",
...
router.post("/login", async (req, res) => {
  ...
  const CPFVerification = await pool.query(
  ...
  if (!CPFVerification.rows[0]) {
    return res
      .status(400)
      .json({ message: "CPF ou senha estão incorretos.", type: "error" });
  } else if (!CPFVerification.rows[0].user_confirmed) {
    return res
      .status(400)
      .json({ message: "Esta conta ainda não foi confirmada. Verifique sua caixa de email (também no spam, lixo ou lixo eletrônico) e encontre seu email de confirmação.", type: "error" });
  }

  if (await bcrypt.compare(password, CPFVerification.rows[0].user_password)) {
    try {
      const token = jwtGenerator(
        CPFVerification.rows[0].user_id,
        CPFVerification.rows[0].user_role,
        CPFVerification.rows[0].user_first_name
      );
      res.json({
        token: token,
    ...
    return res
      .status(403)
      .json({ message: "Usuário ou senha estão incorretos.", type: "error" });
  }
});
...
router.get("/self", authorization, async (req, res) => {
  try {
    const self = await pool.query(
      "SELECT user_email, user_id, user_first_name, user_last_name, user_gender, user_phone, user_cpf, user_birth_date, user_cep, user_state, user_city, user_address, user_number, user_apartment FROM users WHERE user_id = $1",
      [req.userId]
    );
...
router.post("/passwordRequest", async (req, res) => {
  try {
    ...
    const verifyEmail = await pool.query(
      "SELECT user_id, user_email, user_first_name FROM users WHERE user_email = $1",
      [resetEmail]
    );
    const verifyRequests = await pool.query(
      "SELECT reset_id, reset_expiration FROM password_resets WHERE reset_user_id = $1",
    ...
    if (verifyEmail.rows[0]) {
      ...
      const sendEmail = await sgEmail.sendPasswordReset(
        verifyEmail.rows[0].user_first_name,
        newPasswordReset.rows[0].reset_id
      );

      if (sendEmail.type === "error") {
        await pool.query("DELETE FROM password_resets WHERE reset_id = $1", [
          newPasswordReset.rows[0].reset_id,
        ]);
        res.status(400).json({
...
router.get("/passwordRequest/:requestId", async (req, res) => {
  ...
  if (!isValidUuid) {
    return res
      .status(400)
      .json({ message: "Solicitação inválida ou expirada.", type: "error" });
  }

  const now = new Date();
  const verifyRequest = await pool.query(
    "SELECT reset_id, reset_expiration FROM password_resets WHERE reset_id = $1",
    [requestId]
  );
  if (verifyRequest.rows[0]) {
    ...
    return res
      .status(200)
      .json({ message: "Solicitação valida.", type: "success" });
  }
  return res
    .status(400)
    .json({ message: "Solicitação inválida ou expirada.", type: "error" });
...
router.post("/passwordReset/:requestId", async (req, res) => {
  try {
    ...
    const { requestId } = req.params;
    const lookupUser = await pool.query(
      "SELECT reset_user_id FROM password_resets WHERE reset_id = $1",
      [requestId]
    );
    ...
    const updateUser = await pool.query(
      "UPDATE users SET user_password = $1 WHERE user_id = $2 RETURNING user_id, user_role, user_first_name",
      [bcryptPassword, lookupUser.rows[0].reset_user_id]
    );
    const deleteRequest = await pool.query(
      "DELETE FROM password_resets WHERE reset_id = $1",
      [requestId]
    );

    // Logging in
    const token = jwtGenerator(
      updateUser.rows[0].user_id,
      updateUser.rows[0].user_role,
      updateUser.rows[0].user_first_name
    );
    return res.status(200).json({
      token: token,
