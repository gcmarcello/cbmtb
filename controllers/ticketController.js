...
async function list_tickets(req, res) {
  try {
    ...
    res.status(200).json({ message: "Chamados encontrados com sucesso.", type: "success", data: listTickets });
  } catch (err) {
...
async function fetch_ticket(req, res) {
  try {
    ...
    res.status(200).json({ message: "Categoria criada com sucesso!", type: "success", data: ticket });
  } catch (err) {
    console.log(err.message);
  }
}
...
async function answer_ticket(req, res) {
  try {
    ...
    const ticket = await pool.query(`UPDATE tickets SET ticket_status = $1 WHERE ticket_id = $2`, ["completed", id]);
    res.status(200).json({ message: "Chamado respondido com sucesso!", type: "success" });
  } catch (err) {
    console.log(err.message);
    res.status(400).json({ message: `Erro ao responder chamado. ${err.message}`, type: "error" });
  }
}
